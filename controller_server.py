    ...
    while True:
        try:
            if ser.in_waiting > 0:
                ...
                elif message.startswith("SORT"):
                    parts = message.split(',')
                    if len(parts) == 2:
                        config = parts[1].strip()
                        logging.info(f"Processing SORT command with config_a: {config}")
                        print(f"Processing SORT command with config_a: {config}")  # Debugging output
                        response = handle_sort_command(config, local_mode)
                        ser.write(response.encode('utf-8'))

                elif message.startswith("KILL_SORT"):
                    logging.info("Processing KILL_SORT command.")
                    print("Processing KILL_SORT command.")  # Debugging output
                    response = handle_kill_sort_command(local_mode)
                    ser.write(response.encode('utf-8'))
...
def restart_hexa_job(ip):
    ...
    try:
        ...
        private_key_path = "/fasic_home/mstamenk/.ssh/id_rsa_robot43"
        ...
        if '/usr/bin/python3.6 ./hexa_pytest_server.py' in output:
            ...
            ssh.exec_command("cd /home/HGCAL_dev/robot_testing/econd-sw/server; nohup ./hexa_pytest_server.py &")
            return True
...
def handle_kill_sort_command(local_mode):
    try:
        find_command = "pgrep -f chip_sorting.py"
        result = subprocess.run(find_command, shell=True, capture_output=True, text=True)
        if result.returncode == 0:
            process_id = result.stdout.strip()
            kill_command = f"kill {process_id}"
...
def handle_sort_command(config, local_mode):
    try:
        find_command = "pgrep -f chip_sorting.py"
        result = subprocess.run(find_command, shell=True, capture_output=True, text=True)
        if result.returncode == 0:
            logging.info("Script is already running.")
            print("Script is already running.")  # Debugging output
            return "SORTING_ALREADY"
        
        splits = config.split('-')
        inputs = splits[0].split('_')
        outputs = splits[1].split('_')
        chips = splits[2].split('_')

        command = f"nohup ./chip_sorting.py --input_trays {','.join(inputs)} --output_trays {','.join(outputs)} --chips_outputs {','.join(chips)} &"
        os.system(command)
        logging.info(f"Started script with command: {command}")
        print(f"Started script with command: {command}")  # Debugging output
        return "SORT_OK\n"
